    ...
    private static void parallelHatches() {
        ...
        for (int i = 0; i < 5; i+=2) {
            getAssLineResearchBuilder(i+10, 1000, i == 0 ? PARALLEL_HATCH[3].getStackForm() : GCYL_PARALLEL_HATCH[i / 2 - 1].getStackForm(), false, false)
                    ...
                    .input(circuit, getMarkerMaterialByTier(i+11), i == 4 ? 16 : (int) (16 * Math.pow(2, i / 2.0)))
                    .inputs(getSensorByTier(i+10).getStackForm(i == 4 ? 8 : (int) (8 * Math.pow(2, i / 2.0))))
                    .inputs(getEmitterByTier(i+10).getStackForm(i == 4 ? 8 : (int) (8 * Math.pow(2, i / 2.0))))
